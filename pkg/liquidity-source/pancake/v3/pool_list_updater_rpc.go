...
func (d *PoolsListUpdater) fetchPoolData(ctx context.Context, addresses []string) ([]poolRPCData, error) {
	...
	for i := 0; i < len(addresses); i += rpcChunkSize {
		...
		for j, addr := range chunk {
			...
			req.AddCall(&ethrpc.Call{ABI: pancakeV3PoolABI, Target: addr, Method: methodToken0}, []any{token0s[j]})
			req.AddCall(&ethrpc.Call{ABI: pancakeV3PoolABI, Target: addr, Method: methodToken1}, []any{token1s[j]})
			req.AddCall(&ethrpc.Call{ABI: pancakeV3PoolABI, Target: addr, Method: methodFee}, []any{&fees[j]})
			req.AddCall(&ethrpc.Call{ABI: pancakeV3PoolABI, Target: addr, Method: methodTickSpacing}, []any{&tickSpacings[j]})
		}
