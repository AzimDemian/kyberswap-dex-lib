...
// CloneState wins by method promotion unless overridden, silently dropping
// `unsupported`.
func TestCloneState_PreservesUnsupportedGuard(t *testing.T) {
	...
	_, isMcapFee := cloned.(*Hook)
	require.True(t, isMcapFee, "clone must keep dynamic type *mcapfee.Hook, got %T", cloned)
